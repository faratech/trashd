...
fn main() {
    ...
    match cli.command {
        ...
        Commands::Config(subcmd) => cmd_config(subcmd),
...
fn cmd_config(cmd: ConfigCmd) {
    ...
    match cmd {
        ...
        ConfigCmd::Edit => {
            ...
            if !user_path.exists() {
                // Create with defaults so user has something to edit
            ...
            match status {
                Ok(s) if s.success() => {}
                Ok(s) => {
                    eprintln!("{} {editor} exited with {s}", "trash: error:".red().bold());
                    std::process::exit(1);
                }
                Err(e) => {
                    eprintln!("{} launch {editor}: {e}", "trash: error:".red().bold());
                    std::process::exit(1);
                }
            }
        }
        ConfigCmd::Reset { yes } => {
            ...
            if !yes {
                eprint!("Remove {}? [y/N] ", user_path.display(),);
                let _ = std::io::Write::flush(&mut std::io::stderr());
                let mut input = String::new();
                if std::io::stdin().read_line(&mut input).is_err() {
                    return;
...
fn config_get(config: &trashd_common::config::Config, key: &str) -> Option<String> {
    Some(match key {
...
fn load_user_config_table() -> toml::Table {
    let path = trashd_common::config::Config::user_config_path();
...
fn write_user_config_table(table: &toml::Table) {
    ...
    if let Err(e) = std::fs::write(&path, content) {
        eprintln!("{} write config: {e}", "trash: error:".red().bold());
        std::process::exit(1);
    }
}
    ...
    match key {
        "retention.max_age_days" => {
            let v: u32 = value.parse().unwrap_or_else(|_| {
                eprintln!("{} expected integer", "trash: error:".red().bold());
                std::process::exit(1);
            });
            let ret = table
                .entry("retention")
        ...
        "retention.max_size_gb" => {
            let v: f64 = value.parse().unwrap_or_else(|_| {
                eprintln!("{} expected number", "trash: error:".red().bold());
                std::process::exit(1);
            });
            let ret = table
                .entry("retention")
        ...
        "retention.disk_pressure_percent" => {
            let v: u8 = value.parse().unwrap_or_else(|_| {
                eprintln!("{} expected integer 0-100", "trash: error:".red().bold());
                std::process::exit(1);
            });
            let ret = table
                .entry("retention")
        ...
        | "sha256_max_size_mb"
        | "auto_purge_interval_secs" => {
            let v: u64 = value.parse().unwrap_or_else(|_| {
                eprintln!("{} expected integer", "trash: error:".red().bold());
                std::process::exit(1);
            });
        ...
        "hash_algorithm" => {
            if value != "xxhash" && value != "sha256" {
                eprintln!(
                    "{} hash_algorithm must be 'xxhash' or 'sha256'",
                    "trash: error:".red().bold()
                );
                std::process::exit(1);
            }
        ...
        // List keys are handled by add/remove
...
fn config_list_add(table: &mut toml::Table, key: &str, value: &str) {
    match key {
        ...
        _ => {
            eprintln!(
                "{} '{key}' is not a list — use 'trash config set'",
                "trash: error:".red().bold()
            );
            std::process::exit(1);
        }
